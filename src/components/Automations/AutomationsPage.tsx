...
export const AutomationsPage: React.FC = () => {
  ...
  const filteredAutomations = automations
    .filter((auto) => {
      const matchesSearch = auto.name.toLowerCase().includes(search.toLowerCase()) ||
        auto.trigger_config.keywords.some((k) => k.toLowerCase().includes(search.toLowerCase()));
  ...
  const totalRuns = automations.reduce((acc, a) => acc + a.stats.runs, 0);
  const totalDmsSent = automations.reduce((acc, a) => acc + a.stats.dms_sent, 0);
  const totalUniqueUsers = automations.reduce((acc, a) => acc + a.stats.unique_users, 0);
      ...
      <div className="bg-white rounded-2xl p-6 border border-slate-200/80 shadow-2xs space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-extrabold text-slate-900 text-base">Active Automations</h3>
              <p className="text-xs text-slate-500 font-medium">Real-time comment & DM auto-responders running</p>
            </div>
          </div>

          <div className="flex items-center gap-2 bg-emerald-50 border border-emerald-200/80 px-3.5 py-1.5 rounded-full text-xs font-bold text-emerald-700 shrink-0 self-start sm:self-auto">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Total Runs */}
            <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/70 space-y-2 hover:border-indigo-300 transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Total Runs</span>
                <div className="w-8 h-8 rounded-lg bg-indigo-50 text-[#3B5BFF] flex items-center justify-center">
                  <Zap className="w-4 h-4" />
                </div>
              </div>
              <div className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{totalRuns.toLocaleString()}</div>
              <p className="text-[11px] text-slate-500 font-medium">Total automated trigger executions</p>
            </div>

            {/* DMs Sent */}
            <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/70 space-y-2 hover:border-emerald-300 transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">DMs Sent</span>
                <div className="w-8 h-8 rounded-lg bg-emerald-50 text-emerald-600 flex items-center justify-center">
                  <Send className="w-4 h-4" />
                </div>
              </div>
              <div className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{totalDmsSent.toLocaleString()}</div>
              <p className="text-[11px] text-slate-500 font-medium">Direct messages delivered automatically</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* Unique Users */}
            <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/70 space-y-2 hover:border-amber-300 transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Unique Users</span>
                <div className="w-8 h-8 rounded-lg bg-amber-50 text-amber-600 flex items-center justify-center">
                  <Users className="w-4 h-4" />
                </div>
              </div>
              <div className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{totalUniqueUsers.toLocaleString()}</div>
              <p className="text-[11px] text-slate-500 font-medium">Unique Instagram accounts engaged</p>
            </div>

            {/* Comment Replies */}
            <div className="p-4 rounded-xl bg-slate-50/80 border border-slate-200/70 space-y-2 hover:border-purple-300 transition-all">
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Comment Replies</span>
                <div className="w-8 h-8 rounded-lg bg-purple-50 text-purple-600 flex items-center justify-center">
                  <MessageCircle className="w-4 h-4" />
                </div>
              </div>
              <div className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">{totalRuns.toLocaleString()}</div>
              <p className="text-[11px] text-slate-500 font-medium">Automated public post & Reel comment replies</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-2xs space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="relative flex-1 w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-medium focus:ring-2 focus:ring-[#3B5BFF] focus:outline-hidden"
            />
          </div>
          ...
          <button
            ...
            className="w-full sm:w-auto bg-[#3B5BFF] hover:bg-indigo-700 text-white font-bold text-xs py-2.5 px-5 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 shrink-0"
          >
        ...
        <div className="flex flex-wrap items-center gap-3 pt-1 border-t border-slate-100">
          ...
          <select
            ...
            className="bg-slate-50 border border-slate-200 text-slate-700 text-xs font-semibold rounded-xl px-3.5 py-2 focus:ring-2 focus:ring-[#3B5BFF] focus:outline-hidden"
          >
        ...
        {filteredAutomations.length === 0 ? (
          ...
          filteredAutomations.map((auto) => {
            ...
            return (
              <div
                key={auto.id}
                className="bg-white rounded-2xl p-5 md:p-6 border border-slate-200/80 hover:border-slate-300 shadow-2xs hover:shadow-md transition-all space-y-4"
              >
                {/* 1. Header & Name Row */}
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex flex-wrap items-center gap-2.5">
                    ...
                    {/* Name */}
                    <h3 className="text-base font-extrabold text-slate-900 tracking-tight">{auto.name}</h3>

                    {/* Pill Tag */}
                    <span className="bg-emerald-50 text-emerald-700 border border-emerald-200/80 text-[10px] md:text-[11px] font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider">
                      {triggerLabel}
                    </span>
                      ...
                      {openMenuId === auto.id && (
                        <div className="absolute right-0 top-8 bg-white border border-slate-200 rounded-xl shadow-lg w-44 py-1.5 z-20 text-xs font-semibold text-slate-700">
                          ...
                          <button
                            ...
                            className="w-full text-left px-3.5 py-2 hover:bg-slate-50 flex items-center gap-2 text-[#3B5BFF]"
                          >
                ...
                <div className="flex flex-wrap items-center gap-2.5 py-1">
                  ...
                  <div className="flex items-center gap-1.5 bg-indigo-50 border border-indigo-200/80 px-3 py-1.5 rounded-xl text-xs font-bold text-[#3B5BFF]">
                    <Zap className="w-3.5 h-3.5 fill-[#3B5BFF]" />
                    <span>[ ⚡ {auto.actions.length || 1} action ]</span>
                  </div>
                </div>

                {/* 3. Bottom Stats Bar (Metrics Bar) */}
                <div className="pt-3 border-t border-slate-100 flex flex-wrap items-center gap-3 md:gap-5 text-xs font-semibold text-slate-500">
                  <span className="flex items-center gap-1 font-bold text-slate-800">
                    <Zap className="w-3.5 h-3.5 text-emerald-600" />
                    {auto.stats.runs} runs
                  </span>
                  <span>·</span>
                  <span className="flex items-center gap-1">
                    <Users className="w-3.5 h-3.5 text-slate-400" />
                    {auto.stats.unique_users} Users
                  </span>
                  <span>·</span>
